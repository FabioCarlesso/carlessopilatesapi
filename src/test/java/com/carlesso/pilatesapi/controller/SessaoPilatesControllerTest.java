...
class SessaoPilatesControllerTest {
    ...
    void criar_semPacienteId_deveRetornar400() throws Exception {
        var dto = new SessaoPilatesRequestDTO(
                null, null, null, TipoSessao.PILATES, LocalDate.of(2026, 5, 10), null, null, null, null);

        mvc.perform(post("/sessoes").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(dto)))
                .andExpect(status().isBadRequest());
    }

    @Test
    void criar_semTipo_deveRetornar400() throws Exception {
        var dto = new SessaoPilatesRequestDTO(1L, null, null, null, LocalDate.of(2026, 5, 10), null, null, null, null);
    ...
    void criar_semData_deveRetornar400() throws Exception {
        var dto = new SessaoPilatesRequestDTO(1L, null, null, TipoSessao.PILATES, null, null, null, null, null);
    ...
    void criar_comDuracaoNegativa_deveRetornar400() throws Exception {
        var dto = new SessaoPilatesRequestDTO(
                1L, null, null, TipoSessao.PILATES, LocalDate.of(2026, 5, 10), null, null, -10, null);
    ...
    void buscarPorId_quandoNaoExistente_deveRetornar404() throws Exception {
        when(service.buscarPorId(99L)).thenThrow(new ResourceNotFoundException("Sessão não encontrada: 99"));
    ...
    void atualizar_deveRetornar200ComDadosAtualizados() throws Exception {
        ...
        mvc.perform(put("/sessoes/1").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(dto)))
                .andExpect(status().isOk())
    ...
    void atualizar_comSessaoInexistente_deveRetornar404() throws Exception {
        when(service.atualizar(eq(99L), any())).thenThrow(new ResourceNotFoundException("Sessão não encontrada: 99"));

        var dto = new SessaoPilatesUpdateDTO(LocalDate.of(2026, 5, 20), null, null, null, null);

        mvc.perform(put("/sessoes/99").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(dto)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.erro").value("Sessão não encontrada: 99"));
    }
    ...
    void realizar_comSessaoInexistente_deveRetornar404() throws Exception {
        when(service.realizar(99L)).thenThrow(new ResourceNotFoundException("Sessão não encontrada: 99"));
    ...
    void realizar_comTransicaoInvalida_deveRetornar422() throws Exception {
        when(service.realizar(1L))
                .thenThrow(new BusinessException(
                        "Transição inválida: sessão CANCELADA não pode ser alterada para REALIZADA"));

        mvc.perform(patch("/sessoes/1/realizar"))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.erro")
                        .value("Transição inválida: sessão CANCELADA não pode ser alterada para REALIZADA"));
    }
    ...
    void cancelar_comSessaoInexistente_deveRetornar404() throws Exception {
        when(service.cancelar(99L)).thenThrow(new ResourceNotFoundException("Sessão não encontrada: 99"));
    ...
    void cancelar_comTransicaoInvalida_deveRetornar422() throws Exception {
        when(service.cancelar(1L))
                .thenThrow(new BusinessException(
                        "Transição inválida: sessão REALIZADA não pode ser alterada para CANCELADA"));

        mvc.perform(patch("/sessoes/1/cancelar"))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.erro")
                        .value("Transição inválida: sessão REALIZADA não pode ser alterada para CANCELADA"));
    }

    @Test
    void excluir_comSessaoExistente_deveRetornar204() throws Exception {
        mvc.perform(delete("/sessoes/1")).andExpect(status().isNoContent());
    }
