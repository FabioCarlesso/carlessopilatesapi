...
public class PacienteController {
    ...
    @Operation(
            ...
            description =
                    "Registra um novo paciente no sistema. Retorna 201 com o header Location apontando para o recurso criado.")
    @ApiResponses({
    ...
    public ResponseEntity<PacienteResponseDTO> cadastrar(
            @RequestBody @Valid PacienteRequestDTO dto, UriComponentsBuilder uriBuilder) {
        PacienteResponseDTO response = service.cadastrar(dto);
        var uri =
                uriBuilder.path("/pacientes/{id}").buildAndExpand(response.id()).toUri();
    ...
    @Operation(
            ...
            description =
                    "Retorna uma página de pacientes filtrando por nome, e-mail, CPF, telefone e status ativo/inativo. Por padrão retorna pacientes ativos. Suporta paginação e ordenação via query params (page, size, sort).")
    ...
    @Operation(
            ...
            description =
                    "Atualiza os dados de um paciente. Apenas os campos enviados no body serão alterados (atualização parcial).")
    @ApiResponses({
    ...
    @Operation(
            ...
            description =
                    "Realiza soft delete do paciente, marcando-o como inativo. O registro não é removido do banco de dados.")
    @ApiResponses({
