...
- **`min-response-size` só age em resposta com `Content-Length`.** O JSON dos controllers sai em `Transfer-Encoding: chunked` e o Tomcat comprime tamanho desconhecido independentemente do limiar — então toda resposta JSON é comprimida, inclusive as pequenas (`GET /pacientes/3`, 176 bytes crus) e `/auth/**` (367 bytes). Os assets do Swagger UI também são chunked e comprimem (o `swagger-ui.css` transfere 23,8 KB). Quem mantém XLSX, fotos e Actuator fora da compressão é a lista de mime-types, não o limiar (o Actuator responde `application/vnd.spring-boot.actuator.v3+json`)
- **A exceção é o CSV do relatório NFSe**, que devolve `byte[]` com `Content-Length` e por isso é a única resposta desta API efetivamente avaliada pelo limiar: medido em 2026-08-05, um CSV de 99 bytes sai sem compressão
